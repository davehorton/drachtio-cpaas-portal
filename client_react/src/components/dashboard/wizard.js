...
class Wizard extends Component {
  ...
  onForward() {
    ...
    else if (this.state.currentStep === 3) this.props.onCompleted();
  }
  ...
  render() {
    ...
    if (this.state.wizardStarted === false)
      currentElement = (
        <StartWizard onStart={this.onStart} onSkip={this.onSkip} />
      );
    else {
      if (this.state.currentStep === 1)
        currentElement = (
    ...
    return <div className={"align center"}>{currentElement}</div>;
  }
}
